...
# Member 1's input
cap = cv2.VideoCapture('snooker_video3.mp4')

# MEMBER 2 DATA STORAGE
locked_corners = None 
view_status = "UNKNOWN"  # This is the 'Status Flag' for Member 4

# Manual Point storage
manual_points = []
...
def select_points(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN and len(manual_points) < 4:
        ...
        print(f"Point {len(manual_points)} captured at: {x}, {y}")
...
table_locked = False
final_corners = None
stability_counter = 0  # To ensure the table is steady before locking
...
while cap.isOpened():
                    ...
                    if len(approx) == 4:
                        ...
                        if stability_counter > 15:
                            ...
                            if key == ord('y'):
                                ...
                                cv2.setMouseCallback("Manual Mode", select_points)
                                ...
                                final_corners = sort_corners(np.array(manual_points))
                                table_locked = True
                                cv2.destroyWindow("Manual Mode")
                    else:
                        stability_counter = 0 # Reset if the shape is not...
        
        locked_corners = final_corners

    elif green_ratio > 0.85:
        view_status = "ZOOM"
        locked_corners = final_corners # Use the saved coordinates during zoom
    ...
    if locked_corners is not None:
        ...
        sorted_for_mask = sort_corners(locked_corners).astype(np.int32)
    ...
    if locked_corners is not None:
        src_pts = sort_corners(locked_corners)
